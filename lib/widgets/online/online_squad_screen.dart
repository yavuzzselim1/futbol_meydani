...
class _OnlineSquadScreenState extends State<OnlineSquadScreen> {
  ...
  bool get disconnectWinAvailable {
    ...
    return !opponentConnected &&
        lastSeen != null &&
        DateTime.now().toUtc().difference(lastSeen.toUtc()).inSeconds >= 60;
  }
            ...
            if (phase == OnlineSquadPhase.selection) {
              ...
              if (endTime != null) {
                final serverRemaining = max(
                  0,
                  endTime.toUtc().difference(DateTime.now().toUtc()).inSeconds,
                );
  ...
  Future<void> beginSelection() async {
    ...
    try {
      ...
      secondsLeft = existingEnd == null
          ...
          : max(
              0,
              existingEnd.toUtc().difference(DateTime.now().toUtc()).inSeconds,
            );
  ...
  int remainingForOpponent() {
    ...
    return max(0, endTime.toUtc().difference(DateTime.now().toUtc()).inSeconds);
  }
